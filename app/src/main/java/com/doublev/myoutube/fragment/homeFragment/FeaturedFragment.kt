...
class FeaturedFragment : Fragment() {
    ...
    /**
     * Tạo những recommed video cho new feed phần này em chưa tìm được Api nên dùng api
     * của phần search để tạo tạm thời
     */

    private fun setDataForRecyclerView() {
        YoutubeService.youtubeApi
            .getVideosOnSearch(maxResults = 25, regionCode = "VN")
            .enqueue(object : Callback<YoutubeModel<YoutubeSearchResultModel>> {
                override fun onResponse(
                    call: Call<YoutubeModel<YoutubeSearchResultModel>>,
                    response: Response<YoutubeModel<YoutubeSearchResultModel>>
                ) {
                    val data = response.body()
                    if (data != null) {
                        val multipleId = StringBuilder()
                        for (i in data.items) {
                            multipleId.append(i.id.videoId)
                            if (i != data.items.last()) {
                                multipleId.append(",")
                            }
                        }
                        YoutubeService.youtubeApi.getVideosList(
                            id = multipleId.toString(),
                            maxResults = 25
